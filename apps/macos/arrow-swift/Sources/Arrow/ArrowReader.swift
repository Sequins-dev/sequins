...
public class ArrowReader { // swiftlint:disable:this type_body_length
    private class RecordBatchData {
        ...
        private var variadicIndex: Int32 = 0
        ...
        /// Returns the number of variadic data buffers for the next variadic-layout field
        /// (e.g. Utf8View). Advances the variadic cursor.
        func nextVariadicBufferCount() -> Int64 {
            guard variadicIndex < recordBatch.variadicBufferCountsCount else { return 0 }
            defer { variadicIndex += 1 }
            return recordBatch.variadicBufferCounts(at: variadicIndex)
        }
    ...
    -> Result<ArrowArrayHolder, ArrowError> {
        ...
        let arrowNullBuffer = makeBuffer(nullBuffer, fileData: loadInfo.fileData,
                                         length: nullLength, messageOffset: loadInfo.messageOffset)
        // LargeBinary and LargeUtf8 store offsets as Int64 (8 bytes each); Binary and Utf8 use
        // Int32 (4 bytes each). BinaryArray/StringArray read Int32, so convert the offset buffer
        // for large types to avoid reading garbage offset values and crashing.
        let arrowOffsetBuffer: ArrowBuffer
        if field.typeType == .largebinary || field.typeType == .largeutf8 {
            arrowOffsetBuffer = makeLargeOffsetBuffer(
                offsetBuffer,
                fileData: loadInfo.fileData,
                count: Int(node.length) + 1,
                messageOffset: loadInfo.messageOffset
            )
        } else {
            arrowOffsetBuffer = makeBuffer(offsetBuffer, fileData: loadInfo.fileData,
                                           length: UInt(node.length), messageOffset: loadInfo.messageOffset)
        }
        let arrowValueBuffer = makeBuffer(valueBuffer, fileData: loadInfo.fileData,
                                          length: UInt(node.length), messageOffset: loadInfo.messageOffset)
    ...
    /// Loads an Arrow Utf8View (StringView) column. Layout:
    ///   - 1 validity-bitmap buffer
    ///   - 1 views buffer (16 bytes × numRows)
    ///   - N variadic data buffers (count from recordBatch.variadicBufferCounts)
    private func loadVariadicViewData(
        _ loadInfo: DataLoadInfo,
        field: org_apache_arrow_flatbuf_Field)
    -> Result<ArrowArrayHolder, ArrowError> {
        guard let node = loadInfo.batchData.nextNode() else {
            return .failure(.invalid("Node not found for StringView field"))
        }

        guard let nullBuffer = loadInfo.batchData.nextBuffer() else {
            return .failure(.invalid("Null buffer not found for StringView field"))
        }

        guard let viewsBuffer = loadInfo.batchData.nextBuffer() else {
            return .failure(.invalid("Views buffer not found for StringView field"))
        }

        let nullLength = UInt(ceil(Double(node.length) / 8))
        let arrowNullBuffer = makeBuffer(nullBuffer, fileData: loadInfo.fileData,
                                         length: nullLength, messageOffset: loadInfo.messageOffset)
        // Views buffer: fixed 16 bytes per element
        let arrowViewsBuffer = makeBuffer(viewsBuffer, fileData: loadInfo.fileData,
                                          length: UInt(node.length), messageOffset: loadInfo.messageOffset)

        var buffers: [ArrowBuffer] = [arrowNullBuffer, arrowViewsBuffer]

        // Consume the variadic data buffers for this field
        let variadicCount = loadInfo.batchData.nextVariadicBufferCount()
        for _ in 0..<variadicCount {
            guard let dataBuffer = loadInfo.batchData.nextBuffer() else {
                return .failure(.invalid("Variadic data buffer not found for StringView field"))
            }
            buffers.append(makeBuffer(dataBuffer, fileData: loadInfo.fileData,
                                      length: UInt(dataBuffer.length), messageOffset: loadInfo.messageOffset))
        }

        return makeArrayHolder(field, buffers: buffers, nullCount: UInt(node.nullCount),
                               children: nil, rbLength: UInt(loadInfo.batchData.recordBatch.length))
    }
    ...
    -> Result<ArrowArrayHolder, ArrowError> {
        ...
        case .utf8view:
            return loadVariadicViewData(loadInfo, field: field)
        default:
