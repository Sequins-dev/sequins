...
private func makeStringViewHolder(_ buffers: [ArrowBuffer],
                                  nullCount: UInt) -> Result<ArrowArrayHolder, ArrowError> {
    do {
        let arrowType = ArrowType(ArrowType.ArrowStringView)
        let arrowData = try ArrowData(arrowType, buffers: buffers, nullCount: nullCount)
        return .success(ArrowArrayHolderImpl(try StringViewArray(arrowData)))
    } catch let error as ArrowError {
        return .failure(error)
    } catch {
        return .failure(.unknownError("\(error)"))
...
func makeArrayHolder( // swiftlint:disable:this cyclomatic_complexity
    ...
    case .stringView:
        return makeStringViewHolder(buffers, nullCount: nullCount)
    case .binary:
...
/// Converts a LargeBinary/LargeUtf8 Int64 offset buffer to Int32 so that BinaryArray/StringArray
/// can read offsets without crashing. LargeBinary uses 8-byte offsets; Binary uses 4-byte offsets.
/// Values that fit in Int32 range are preserved exactly (truncating upper 32 bits which are 0).
func makeLargeOffsetBuffer(
    _ buffer: org_apache_arrow_flatbuf_Buffer,
    fileData: Data,
    count: Int,
    messageOffset: Int64
) -> ArrowBuffer {
    let startByte = Int(messageOffset + buffer.offset)
    let endByte = min(startByte + Int(buffer.length), fileData.count)
    var int32Bytes = [UInt8](repeating: 0, count: count * MemoryLayout<Int32>.stride)
    for i in 0..<count {
        let byteOffset = startByte + i * MemoryLayout<Int64>.stride
        guard byteOffset + MemoryLayout<Int64>.stride <= endByte else { break }
        let int64Value = fileData.withUnsafeBytes { ptr in
            ptr.loadUnaligned(fromByteOffset: byteOffset, as: Int64.self)
        }
        let int32Value = Int32(truncatingIfNeeded: int64Value)
        let base = i * MemoryLayout<Int32>.stride
        withUnsafeBytes(of: int32Value) { src in
            for j in 0..<MemoryLayout<Int32>.stride {
                int32Bytes[base + j] = src[j]
            }
        }
    }
    return ArrowBuffer.createBuffer(int32Bytes, length: UInt(count - 1))
}
...
func findArrowType( // swiftlint:disable:this cyclomatic_complexity function_body_length
    ...
    case .utf8view:
        return ArrowType(ArrowType.ArrowStringView)
    case .largeutf8:
        // LargeUtf8 uses 64-bit offsets; treated as Utf8 for values that fit within 32-bit offset range
        return ArrowType(ArrowType.ArrowString)
    case .binary:
        return ArrowType(ArrowType.ArrowBinary)
    case .largebinary:
        // LargeBinary uses 64-bit offsets; treated as Binary for values that fit within 32-bit offset range
        return ArrowType(ArrowType.ArrowBinary)
    case .date:
    ...
    case .map:
        let mapMeta = field.type(type: org_apache_arrow_flatbuf_Map.self)
        let keysSorted = mapMeta?.keysSorted ?? false
        // Map has one child: the entries struct, which has key and value children
        guard field.childrenCount == 1, let entriesField = field.children(at: 0),
              entriesField.childrenCount >= 2,
              let keyFlatField = entriesField.children(at: 0),
              let valueFlatField = entriesField.children(at: 1) else {
            return ArrowType(ArrowType.ArrowUnknown)
        }
        let keyType = findArrowType(keyFlatField)
        let valueType = findArrowType(valueFlatField)
        let keyField = ArrowField(keyFlatField.name ?? "key", type: keyType, isNullable: keyFlatField.nullable)
        let valueField = ArrowField(valueFlatField.name ?? "value", type: valueType, isNullable: valueFlatField.nullable)
        return ArrowTypeMap(keyField: keyField, valueField: valueField, keysSorted: keysSorted)
    default:
        return ArrowType(ArrowType.ArrowUnknown)
    }
}
