...
func toFBTypeEnum(_ arrowType: ArrowType) -> Result<org_apache_arrow_flatbuf_Type_, ArrowError> {
    ...
    case .map:
        return .success(org_apache_arrow_flatbuf_Type_.map)
    default:
...
func toFBType( // swiftlint:disable:this cyclomatic_complexity function_body_length
    ...
    case .map:
        if let mapType = arrowType as? ArrowTypeMap {
            return .success(org_apache_arrow_flatbuf_Map.createMap(&fbb, keysSorted: mapType.keysSorted))
        }
        return .failure(.invalid("Unable to cast to ArrowTypeMap"))
    default:
