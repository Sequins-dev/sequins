...
func fromProto( // swiftlint:disable:this cyclomatic_complexity function_body_length
    ...
    case .utf8view:
        arrowType = ArrowType(ArrowType.ArrowStringView)
    case .largeutf8:
    ...
    case .map:
        ...
        let keyArrowField = fromProto(field: keyFlatField)
        let valueArrowField = fromProto(field: valueFlatField)
        let mapMeta = field.type(type: org_apache_arrow_flatbuf_Map.self)
        arrowType = ArrowTypeMap(
            keyField: keyArrowField,
            valueField: valueArrowField,
            keysSorted: mapMeta?.keysSorted ?? false
        )
    default:
        arrowType = ArrowType(ArrowType.ArrowUnknown)
    }
