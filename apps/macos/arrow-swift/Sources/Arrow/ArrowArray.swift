...
public class ArrowArrayHolderImpl: ArrowArrayHolder {
    ...
    public static func loadArray( // swiftlint:disable:this cyclomatic_complexity
        ...
        case .stringView:
            return try ArrowArrayHolderImpl(StringViewArray(with))
        case .boolean:
...
/// Reads Arrow StringView (Utf8View) encoded columns.
///
/// Each element is a 16-byte view:
///   - Bytes 0–3:  Int32 `length`
///   - If `length` ≤ 12 (inline):  bytes 4–(4+length) hold the UTF-8 data directly
///   - If `length` > 12 (out-of-line): bytes 4–7 prefix, bytes 8–11 Int32 `bufferIndex`,
///     bytes 12–15 Int32 `byteOffset` into the variadic data buffer at `buffers[2+bufferIndex]`
public class StringViewArray: ArrowArray<String> {
    public override subscript(_ index: UInt) -> String? {
        if self.arrowData.isNull(index) { return nil }
        let viewOffset = Int(index) * 16
        let viewsPtr = self.arrowData.buffers[1].rawPointer
        let length = viewsPtr.advanced(by: viewOffset).load(as: Int32.self)
        guard length >= 0 else { return nil }
        if length <= 12 {
            // Inline: the string bytes follow the length field at byte 4
            let rawPtr = viewsPtr.advanced(by: viewOffset + 4)
                .bindMemory(to: UInt8.self, capacity: Int(length))
            return String(data: Data(UnsafeBufferPointer(start: rawPtr, count: Int(length))), encoding: .utf8)
        } else {
            // Out-of-line: look up the variadic data buffer
            let bufferIndex = viewsPtr.advanced(by: viewOffset + 8).load(as: Int32.self)
            let byteOffset = viewsPtr.advanced(by: viewOffset + 12).load(as: Int32.self)
            guard 2 + Int(bufferIndex) < self.arrowData.buffers.count else { return nil }
            let dataPtr = self.arrowData.buffers[2 + Int(bufferIndex)].rawPointer
            let rawPtr = dataPtr.advanced(by: Int(byteOffset))
                .bindMemory(to: UInt8.self, capacity: Int(length))
            return String(data: Data(UnsafeBufferPointer(start: rawPtr, count: Int(length))), encoding: .utf8)
        }
    }
...
public class TimestampArray: FixedArray<Timestamp> {
    ...
    func dateFromTimestamp(_ timestamp: Int64, unit: ArrowTimestampUnit) -> Date {
    ...
    /// Returns a `Date` instead of the raw `Int64` timestamp, enabling direct Codable decoding.
    public override func asAny(_ index: UInt) -> Any? {
        guard let timestamp = self[index] else { return nil }
        guard let tsType = self.arrowData.type as? ArrowTypeTimestamp else {
            return timestamp
        }
        return dateFromTimestamp(timestamp, unit: tsType.unit)
    }
...
public class NestedArray: ArrowArray<[Any?]> {
    ...
    public required init(_ arrowData: ArrowData) throws {
        ...
        case .map:
            // Map is physically List<Struct<key, value>>. Load the one entries-struct child.
            guard arrowData.children.count == 1 else {
                throw ArrowError.invalid("Map array must have exactly one child (entries struct)")
            }
            let entriesData = arrowData.children[0]
            self.children = [try ArrowArrayHolderImpl.loadArray(entriesData.type, with: entriesData)]

        default:
            throw ArrowError.invalid("NestedArray only supports list, struct, and map types, got: \(arrowData.type.id)")
        }
    }
    ...
    public override subscript(_ index: UInt) -> [Any?]? {
        ...
        case .map:
            // Same physical layout as list: offsets + one entries-struct child
            guard let entries = children.first else { return nil }
            ...
            var items = [Any?]()
            for i in startOffset..<endOffset {
                items.append(entries.array.asAny(UInt(i)))
            }
    ...
    public var isMapArray: Bool {
        return arrowData.type.id == .map
    }
    ...
    /// For Map arrays: the entries-struct holder containing key/value children.
    public var entries: ArrowArrayHolder? {
        return arrowData.type.id == .map ? children?.first : nil
    }

    /// Entry range [start, end) for a given row index (valid for .list and .map).
    public func entryRange(at rowIndex: UInt) -> (Int32, Int32)? {
        guard arrowData.type.id == .list || arrowData.type.id == .map,
              arrowData.buffers.count >= 2 else { return nil }
        let offsets = arrowData.buffers[1]
        let base = Int(rowIndex) * MemoryLayout<Int32>.stride
        let start = offsets.rawPointer.advanced(by: base).load(as: Int32.self)
        let end = offsets.rawPointer.advanced(by: base + MemoryLayout<Int32>.stride).load(as: Int32.self)
        return (start, end)
    }
}
