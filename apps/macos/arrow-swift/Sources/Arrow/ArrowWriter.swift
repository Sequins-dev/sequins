...
public class ArrowWriter { // swiftlint:disable:this type_body_length
    ...
    private func writeField(_ fbb: inout FlatBufferBuilder, field: ArrowField) -> Result<Offset, ArrowError> {
        ...
        } else if let mapType = field.type as? ArrowTypeMap {
            // Map has one child: the entries struct with key and value children
            let entriesFields = [mapType.keyField, mapType.valueField]
            let entriesType = ArrowTypeStruct(ArrowType.ArrowStruct, fields: entriesFields)
            let entriesField = ArrowField("entries", type: entriesType, isNullable: false)
            switch writeField(&fbb, field: entriesField) {
            case .success(let offset):
    ...
    private func writeFieldNodes(_ fields: [ArrowField], columns: [ArrowArrayHolder], offsets: inout [Offset],
        ...
        for index in (0 ..< fields.count).reversed() {
            ...
            if let nestedType = column.type as? ArrowTypeStruct {
                ...
                // Write FieldNode for the list child element
                let nestedArray = column.array as? NestedArray
                if let values = nestedArray?.values {
                    writeFieldNodes([listType.elementField], columns: [values], offsets: &offsets, fbb: &fbb)
                }
            } else if let mapType = column.type as? ArrowTypeMap {
                // Map child is the entries struct; write its FieldNode then key/value FieldNodes
                let nestedArray = column.array as? NestedArray
                if let entriesHolder = nestedArray?.entries {
                    let entriesField = ArrowField("entries",
                        type: ArrowTypeStruct(ArrowType.ArrowStruct,
                                             fields: [mapType.keyField, mapType.valueField]),
                        isNullable: false)
                    writeFieldNodes([entriesField], columns: [entriesHolder], offsets: &offsets, fbb: &fbb)
                }
            }
        }
    }
    ...
    private func writeBufferInfo(_ fields: [ArrowField],
        ...
        for index in 0 ..< fields.count {
            ...
            // Write this column's own buffers first
            ...
            // Recurse into children AFTER all own buffers
            if let nestedType = column.type as? ArrowTypeStruct {
                let nestedArray = column.array as? NestedArray
                if let nestedFields = nestedArray?.fields {
                ...
                let nestedArray = column.array as? NestedArray
                if let values = nestedArray?.values {
                    writeBufferInfo([listType.elementField], columns: [values],
                                    bufferOffset: &bufferOffset, buffers: &buffers, fbb: &fbb)
                }
            } else if let mapType = column.type as? ArrowTypeMap {
                let nestedArray = column.array as? NestedArray
                if let entriesHolder = nestedArray?.entries {
                    let entriesField = ArrowField("entries",
                        type: ArrowTypeStruct(ArrowType.ArrowStruct,
                                             fields: [mapType.keyField, mapType.valueField]),
                        isNullable: false)
                    writeBufferInfo([entriesField], columns: [entriesHolder],
                                    bufferOffset: &bufferOffset, buffers: &buffers, fbb: &fbb)
                }
            }
        }
    ...
    private func writeRecordBatchData(
        ...
        for index in 0 ..< fields.count {
            ...
            if let nestedType = column.type as? ArrowTypeStruct {
                ...
                guard let nestedArray = column.array as? NestedArray,
                      let values = nestedArray.values else {
                    return .failure(.invalid("List type array expected child values"))
                }
                switch writeRecordBatchData(&writer, fields: [listType.elementField], columns: [values]) {
                case .success: break
                case .failure(let error): return .failure(error)
                }
            } else if let mapType = column.type as? ArrowTypeMap {
                guard let nestedArray = column.array as? NestedArray,
                      let entriesHolder = nestedArray.entries else {
                    return .failure(.invalid("Map type array expected entries struct"))
                }
                let entriesField = ArrowField("entries",
                    type: ArrowTypeStruct(ArrowType.ArrowStruct,
                                         fields: [mapType.keyField, mapType.valueField]),
                    isNullable: false)
                switch writeRecordBatchData(&writer, fields: [entriesField], columns: [entriesHolder]) {
                case .success: break
                case .failure(let error): return .failure(error)
                }
            }
