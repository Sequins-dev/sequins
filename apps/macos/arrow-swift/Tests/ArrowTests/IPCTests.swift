...
// Simple class used to create a 2-field struct for testdata_struct.arrow
private class TwoFieldStruct {
    var text: String = ""
    var flag: Bool = false
}

private func generateBoolTestData(to url: URL) throws {
    let boolBuilder = try ArrowArrayBuilders.loadBoolArrayBuilder()
    boolBuilder.append(true)
    boolBuilder.append(false)
    boolBuilder.append(Bool?.none)
    boolBuilder.append(false)
    boolBuilder.append(true)

    let stringBuilder = try ArrowArrayBuilders.loadStringArrayBuilder()
    stringBuilder.append("val0")
    stringBuilder.append("val1")
    stringBuilder.append("val2")
    stringBuilder.append("val3")
    stringBuilder.append("val4")

    let boolHolder = ArrowArrayHolderImpl(try boolBuilder.finish())
    let stringHolder = ArrowArrayHolderImpl(try stringBuilder.finish())
    let batchResult = RecordBatch.Builder()
        .addColumn("one", arrowArray: boolHolder)
        .addColumn("two", arrowArray: stringHolder)
        .finish()
    guard case .success(let batch) = batchResult else { return }
    let writerInfo = ArrowWriter.Info(.recordbatch, schema: batch.schema, batches: [batch])
    if case .failure(let error) = ArrowWriter().toFile(url, info: writerInfo) { throw error }
}

private func generateDoubleTestData(to url: URL) throws {
    let doubleBuilder: NumberArrayBuilder<Double> = try ArrowArrayBuilders.loadNumberArrayBuilder()
    doubleBuilder.append(1.1)
    doubleBuilder.append(2.2)
    doubleBuilder.append(3.3)
    doubleBuilder.append(4.4)
    doubleBuilder.append(5.5)

    let stringBuilder = try ArrowArrayBuilders.loadStringArrayBuilder()
    stringBuilder.append("str0")
    stringBuilder.append(String?.none)
    ...
    let stringHolder = ArrowArrayHolderImpl(try stringBuilder.finish())
    let batchResult = RecordBatch.Builder()
        .addColumn("one", arrowArray: doubleHolder)
        .addColumn("two", arrowArray: stringHolder)
        .finish()
    guard case .success(let batch) = batchResult else { return }
    let writerInfo = ArrowWriter.Info(.recordbatch, schema: batch.schema, batches: [batch])
...
private func generateStructTestData(to url: URL) throws {
    let structBuilder = try ArrowArrayBuilders.loadStructArrayBuilderForType(TwoFieldStruct())
    structBuilder.append(["0", false])
    structBuilder.append(["1", true])
    structBuilder.append([Any?]?.none)

    let structHolder = ArrowArrayHolderImpl(try structBuilder.finish())
    let batchResult = RecordBatch.Builder()
        .addColumn("my struct", arrowArray: structHolder)
        .finish()
    guard case .success(let batch) = batchResult else { return }
    let writerInfo = ArrowWriter.Info(.recordbatch, schema: batch.schema, batches: [batch])
    if case .failure(let error) = ArrowWriter().toFile(url, info: writerInfo) { throw error }
...
final class IPCFileReaderTests: XCTestCase { // swiftlint:disable:this type_body_length
    override class func setUp() {
        ...
        let boolURL = testDir.appendingPathComponent("testdata_bool.arrow")
        if !fm.fileExists(atPath: boolURL.path) { try? generateBoolTestData(to: boolURL) }
        let doubleURL = testDir.appendingPathComponent("testdata_double.arrow")
        if !fm.fileExists(atPath: doubleURL.path) { try? generateDoubleTestData(to: doubleURL) }
        let structURL = testDir.appendingPathComponent("testdata_struct.arrow")
        if !fm.fileExists(atPath: structURL.path) { try? generateStructTestData(to: structURL) }
    }
