...
use sequins::{
    error::Result,
    models::{
        AttributeValue, Duration, HistogramDataPointQueryResult, LogEntry, LogId, LogQuery,
        LogQueryResult, LogSeverity, Metric, MetricDataPointQueryResult, MetricId, MetricQuery,
        MetricQueryResult, MetricType, Profile, ProfileId, ProfileQuery, ProfileQueryResult,
        ProfileSample, ProfileSampleQuery, ProfileSampleQueryResult, ProfileType, QueryCursor,
        QueryTrace, Service, Span, SpanId, SpanKind, SpanQuery, SpanQueryResult, SpanStatus,
        Timestamp, TraceId, TraceQuery, TraceQueryResult,
    },
    traits::QueryApi,
};
...
/// Type alias for spans lookup map
type SpansMap = HashMap<(TraceId, Option<SpanId>), Vec<Span>>;
...
impl QueryApi for MockQueryApi {
    ...
    async fn subscribe_metric_data_points(
        ...
        _query: sequins::models::MetricDataPointQuery,
        _cursor: QueryCursor,
    ) -> Result<Pin<Box<dyn Stream<Item = sequins::models::MetricDataPoint> + Send>>> {
        Ok(Box::pin(futures::stream::iter(vec![])))
    }
...
async fn test_query_traces() {
    ...
    let result: TraceQueryResult = response.json().await.unwrap();
    assert_eq!(result.traces.len(), 1);
    assert_eq!(result.traces[0].trace_id, trace_id);
}
...
async fn test_query_logs() {
    ...
    let result: LogQueryResult = response.json().await.unwrap();
...
async fn test_query_metrics() {
    ...
    let result: MetricQueryResult = response.json().await.unwrap();
...
async fn test_query_profiles() {
    ...
    let response = client
        .post("http://127.0.0.1:18080/api/profiles")
        .json(&query)
        .send()
        .await
        .unwrap();
    ...
    let result: ProfileQueryResult = response.json().await.unwrap();
    assert_eq!(result.profiles.len(), 1);
    assert_eq!(result.profiles[0].service_name, "test-service");
...
async fn test_get_profile() {
    ...
    let profile: Option<Profile> = response.json().await.unwrap();
    assert!(profile.is_some());
    let profile = profile.unwrap();
    assert_eq!(profile.profile_type, ProfileType::Memory);
    assert_eq!(profile.data, vec![5, 6, 7, 8]);
}
...
#[tokio::test]
#[serial]
async fn test_invalid_trace_id() {
    let (_mock, _handle) = start_query_server().await;

    let client = reqwest::Client::new();
    let response = client
        .get("http://127.0.0.1:18080/api/traces/invalid-hex/spans")
        .send()
        .await
        .unwrap();
    ...
    assert!(body["error"].as_str().unwrap().contains("Invalid trace ID"));
}

#[tokio::test]
#[serial]
async fn test_invalid_span_id() {
    let (_mock, _handle) = start_query_server().await;
    ...
    let response = client
        .get(format!(
            "http://127.0.0.1:18080/api/traces/{}/spans/not-a-valid-hex",
            trace_id.to_hex()
        ))
        .send()
        .await
    ...
    let body: serde_json::Value = response.json().await.unwrap();
    assert!(body["error"].as_str().unwrap().contains("Invalid span ID"));
}

#[tokio::test]
#[serial]
async fn test_invalid_log_id() {
    let (_mock, _handle) = start_query_server().await;

    let client = reqwest::Client::new();
    let response = client
        .get("http://127.0.0.1:18080/api/logs/bad-id-format")
        .send()
        .await
        .unwrap();
    ...
    let body: serde_json::Value = response.json().await.unwrap();
    assert!(body["error"].as_str().unwrap().contains("Invalid log ID"));
}

#[tokio::test]
#[serial]
async fn test_invalid_metric_id() {
    let (_mock, _handle) = start_query_server().await;

    let client = reqwest::Client::new();
    let response = client
        .get("http://127.0.0.1:18080/api/metrics/xyz123")
        .send()
        .await
        .unwrap();
...
async fn test_invalid_profile_id() {
    ...
    let client = reqwest::Client::new();
    let response = client
        .get("http://127.0.0.1:18080/api/profiles/not-valid")
        .send()
        .await
        .unwrap();
