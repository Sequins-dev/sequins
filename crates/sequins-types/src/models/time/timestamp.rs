...
pub enum TimestampError {
    ...
    /// SystemTime is outside the representable range for nanosecond timestamps
    SystemTimeOutOfRange,

    #[error("Timestamp {0} is out of representable range")]
    /// Timestamp value in nanoseconds is outside valid range
    OutOfRange(i64),
}
...
impl Timestamp {
    ...
    pub fn duration_since(&self, earlier: Timestamp) -> Duration {
        debug_assert!(
            self.0 >= earlier.0,
            "duration_since called with self ({}) < earlier ({}); result would underflow",
            self.0,
            earlier.0
        );
...
mod tests {
    ...
    #[should_panic(expected = "duration_since called with self")]
    fn test_timestamp_duration_since_ordering_violation_panics_in_debug() {
        // Calling duration_since where self < earlier is a logic error;
        // in debug builds the debug_assert! should fire.
        let earlier = Timestamp::from_secs(1100);
        let later = Timestamp::from_secs(1000);
        let _ = later.duration_since(earlier); // self < earlier → panic in debug
    }
}
