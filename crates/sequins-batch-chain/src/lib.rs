...
impl BatchChain {
    ...
    pub fn head_arc(&self) -> Arc<Atomic<BatchNode>> {
        Arc::clone(&self.head)
    }
...
///
/// Type-erased cold flush callback.
///
/// Called with the completed `RecordBatch` when a node is evicted from the
/// hot-tier chain.  Using `Box<dyn Fn(...) -> BoxFuture<...>>` lets the
/// hot tier stay free of any cold-tier dependency.
...
/// `cold_writer_fn` is an optional callback invoked with the completed
/// `RecordBatch` when a node is ready to be flushed to cold storage.
/// Pass `None` to disable flushing (useful for tests or when the
