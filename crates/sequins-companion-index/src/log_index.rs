...
use crate::span_index::IndexStrategy;
...
pub struct LogCompanionIndex {
    /// In-memory Tantivy index — kept alive for `has_match` / `has_attribute_match` queries.
    tantivy_index: Index,
...
impl LogCompanionIndex {
    ...
    pub fn build_for_batch(batch: &RecordBatch, bloom_fpr: f64) -> Result<Self, String> {
        if batch.num_rows() == 0 {
        ...
        // Step 2: Create in-memory Tantivy index — no filesystem round-trip.
        ...
        Self::build_bloom_filters_from_batch(&mut bloom_filters, batch, bloom_fpr);
    ...
    pub fn into_companion_bytes(self) -> Result<crate::CompanionIndexBytes, String> {
        ...
        // Extract all index files from the in-memory directory.
        let tantivy_files = crate::collect_tantivy_files_from_index(&self.tantivy_index)?;
...
mod tests {
    ...
    fn test_log_index_build() {
        ...
        let index = LogCompanionIndex::build_for_batch(&batch, 0.01).unwrap();
    ...
    fn test_log_index_empty_logs() {
        ...
        let result = LogCompanionIndex::build_for_batch(&empty, 0.01);
