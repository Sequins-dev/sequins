...
pub struct SpanCompanionIndex {
    /// Serialized Tantivy index files extracted from the RAM directory.
    /// `None` means no Tantivy index was built (no indexable attributes).
    tantivy_files: Option<Vec<(String, Vec<u8>)>>,
...
impl SpanCompanionIndex {
    ...
    pub fn build_for_batch(
        ...
        // Step 4: Build the Tantivy index entirely in RAM — no filesystem round-trip.
        let tantivy_files = {
            let tantivy_index = Index::create_in_ram(schema.clone());
            let mut index_writer = tantivy_index
                .writer(50_000_000) // 50 MB write buffer
                .map_err(|e| format!("Failed to create Tantivy writer: {}", e))?;

            // Step 5: Index all rows
            ...
            // Extract committed files from the RAM directory before the index is dropped.
            crate::collect_tantivy_files_from_index(&tantivy_index)
                .map_err(|e| format!("Failed to collect Tantivy files: {}", e))?
        };

        // Step 6: Build bloom filters for BloomOnly fields
        let mut bloom_filters = BloomFilterSet::new();
        Self::build_bloom_filters_from_batch(&mut bloom_filters, batch, &strategies, bloom_fpr);
        ...
        Ok(Self {
            tantivy_files: Some(tantivy_files),
            bloom_filters,
            metadata,
        })
    }
    ...
    pub fn into_companion_bytes(self) -> Result<crate::CompanionIndexBytes, String> {
        let bloom_bytes = self.bloom_filters.serialize()?;
        let tantivy_files = self.tantivy_files.unwrap_or_default();

        Ok(crate::CompanionIndexBytes {
            bloom_bytes,
...
mod tests {
    ...
    fn test_cardinality_classification() {
        ...
        let index = SpanCompanionIndex::build_for_batch(&batch, 100, 0.01).unwrap();
