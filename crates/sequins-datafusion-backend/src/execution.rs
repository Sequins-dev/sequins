...
async fn execute_snapshot(
    ...
    total_rows += execute_aux_plans(&ctx, &ext, watermark, &mut all_frames).await?;
...
///
/// This is the common inner step shared by the Replace live-stream re-execution
/// and any other path that needs a full plan → batch round-trip.
async fn consume_and_execute_root(
    ctx: &SessionContext,
    plan_bytes: &[u8],
) -> Result<arrow::record_batch::RecordBatch, QueryError> {
    let plan: Plan =
        Message::decode(plan_bytes).map_err(|e| exec_err(format!("Substrait decode: {}", e)))?;
...
/// frames to `frames`.
///
/// Each auxiliary plan is decoded independently so its own extension context is
/// preserved (avoids "function reference not found" errors that occur when
/// auxiliary relations are merged into the primary plan).
async fn execute_aux_plans(
    ctx: &SessionContext,
    ext: &seql_ext::SeqlExtension,
    watermark: u64,
    frames: &mut Vec<Result<FlightData, QueryError>>,
    ...
    for (idx, aux_bytes) in ext.auxiliary_plan_bytes.iter().enumerate() {
        ...
        if batch.num_rows() > 0 {
            frames.push(Ok(data_flight_data(alias, &batch)));
        }
    }
    Ok(rows)
}
...
async fn execute_live(
    ...
    // Execute each auxiliary plan independently using its own extensions context.
    execute_aux_plans(&ctx, &ext, watermark, &mut historical_frames).await?;
        ...
        loop {
            ...
            tokio::select! {
                ...
                _ = async {
                    ...
                    match consume_and_execute_root(&ctx, &plan_bytes).await {
                    ...
                    for (idx, aux_bytes) in auxiliary_plan_bytes.iter().enumerate() {
                        ...
                        match consume_and_execute_root(&ctx, aux_bytes).await {
...
mod tests {
    ...
    async fn test_query_execution_against_hot_tier() {
        ...
        assert_eq!(hot_tier.chain(&SignalType::Spans).row_count(), 10);
